...
object FileUtils {
    ...
    fun getMimeType(file: File): String? {
        ...
        return if (!extension.isNullOrEmpty()) MimeTypeMap.getSingleton().getMimeTypeFromExtension(extension.substring(1)) else "application/octet-stream"
    }
    ...
    fun getThumbnail(context: Context, file: File): Bitmap? {
        return getThumbnail(context, getUri(file), getMimeType(file) ?: Constants.EMPTY_STRING)
    }
    ...
    fun getThumbnail(context: Context, uri: Uri?): Bitmap? {
        return uri?.let {
            getThumbnail(context, uri, getMimeType(context, uri) ?: Constants.EMPTY_STRING)
        }
    }
     ...
     * @author paulburke
     */
    var sComparator: Comparator<File> = Comparator { f1, f2 -> // Sort alphabetically by lower case, which is much cleaner
    ...
    fun getMediaPathFromUri(uri: Uri, context: Context): String? {
