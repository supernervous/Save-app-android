...
class MediaViewHolder(
    ...
    private var asyncTask: AsyncTask<Void, Void, SoundFile>? = null
    var mSoundFileCache = HashMap<String, SoundFile>()
    ...
    fun bindData(currentMedia: Media, isBatchMode: Boolean) {
        ...
        if (lastMediaPath == null || lastMediaPath != mediaPath) {
            if (currentMedia.mimeType.startsWith("image")) {
                mPicasso?.let {
                    it.load(Uri.parse(currentMedia.originalFilePath)).fit().centerCrop().into(ivIcon)
                ...
                mPicasso?.let {
                    it.load(VideoRequestHandler.SCHEME_VIDEO + ":" + currentMedia.originalFilePath).fit().centerCrop().into(ivIcon)
                    ivIcon.visibility = View.VISIBLE
                    tvWave.visibility = View.GONE
                    ivIsVideo.visibility = View.VISIBLE
                }
            } else if (currentMedia.mimeType.startsWith("audio")) {
                ivIcon.setImageDrawable(ContextCompat.getDrawable(mContext, R.drawable.no_thumbnail))
                ivIsVideo.visibility = View.GONE
                if (mSoundFileCache[mediaPath] == null) {
                    if (asyncTask == null) {
                        asyncTask = object : AsyncTask<Void, Void, SoundFile>() {

                            override fun onPreExecute() {
                                // NO-OP
                            }

                            override fun doInBackground(vararg params: Void?): SoundFile {
                                ...
                                return SoundFile.create("") {
                                }
                            }

                            override fun onPostExecute(soundFile: SoundFile?) {
                                // Post Code
                                if (soundFile != null) {
            ...
            if (fileMedia.exists()) {
                ...
                if (currentMedia.contentLength == -1L) {
                    try {
                        val `is` = mContext.contentResolver.openInputStream(Uri.parse(currentMedia.originalFilePath))
                ...
                if (currentMedia.contentLength > 0) tvCreateDate.text = readableFileSize(currentMedia.contentLength) else tvCreateDate.text = currentMedia.getFormattedCreateDate()
            }
        }
        ...
        if (currentMedia.status == Media.STATUS_ERROR) {
            ...
            if (!TextUtils.isEmpty(currentMedia.statusMessage)) tvCreateDate.text = currentMedia.statusMessage
            ...
            var perc = 0
            if (currentMedia.contentLength > 0) perc = (currentMedia.progress.toFloat() / currentMedia.contentLength.toFloat() * 100f).toInt()
            progressBar?.let { progressBar ->
                progressBar.visibility = View.VISIBLE
                tvProgress.visibility = View.VISIBLE
        ...
        if (!TextUtils.isEmpty(currentMedia.location)) ivEditLocation.setImageResource(R.drawable.ic_location_selected) else ivEditLocation.setImageResource(R.drawable.ic_location_unselected)
        if (!TextUtils.isEmpty(currentMedia.getTags())) ivEditTags.setImageResource(R.drawable.ic_tag_selected) else ivEditTags.setImageResource(R.drawable.ic_tag_unselected)
        if (!TextUtils.isEmpty(currentMedia.description)) ivEditNotes.setImageResource(R.drawable.ic_edit_selected) else ivEditNotes.setImageResource(R.drawable.ic_edit_unselected)
        if (currentMedia.flag) ivEditFlag.setImageResource(R.drawable.ic_flag_selected) else ivEditFlag.setImageResource(R.drawable.ic_flag_unselected)
    }
